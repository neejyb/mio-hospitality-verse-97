        ...
        <div className="mobile-service-grid">
          {features.map((feature, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className={`mobile-service-card rounded-lg shadow-lg ${darkMode ? 'bg-[#4a1403]/80' : 'bg-white'}`}
            >
              <div className="flex items-start mb-3 sm:mb-4">
                <div className="mr-3 sm:mr-4 text-[#D4AF37]">
                  {feature.icon || <Check size={16} className="sm:w-5 sm:h-5" />}
                </div>
                <h3 className={`text-lg sm:text-xl font-bold ${textColor}`}>{feature.title}</h3>
              </div>
              <p className={`${descriptionColor} responsive-body`}>{feature.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
