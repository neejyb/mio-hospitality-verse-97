...
const ServiceTestimonials = ({
  ...
  return <section className={`mobile-testimonial-section ${bgColor}`}>
      <div className="container mx-auto mobile-container-padding">
        ...
        <div className="max-w-5xl mx-auto" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
          <div className="overflow-hidden" ref={emblaRef}>
            <div className="flex">
              {testimonials.map((testimonial, index) => <div key={testimonial.id} className="flex-[0_0_100%] md:flex-[0_0_50%] pl-2 sm:pl-4">
                  <motion.div initial={{
                opacity: 0,
                y: 20
              }} whileInView={{
                ...
                once: true
                    ...
                    <div className="flex flex-col h-full">
                      ...
                      <div className="flex items-center">
                        {testimonial.avatar && <div className="mr-2 sm:mr-4">
                            <img src={testimonial.avatar} alt={testimonial.author} className="rounded-full w-8 h-8 sm:w-12 sm:h-12 object-cover" />
                          </div>}
                        <div>
                          <h4 className={`font-bold mobile-text-sm ${textColor}`}>{testimonial.author}</h4>
                          {(testimonial.position || testimonial.company) && <p className={`text-xs sm:text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
          ...
          <div className="mt-4 sm:mt-8 flex justify-center gap-1 sm:gap-2">
            {testimonials.map((_, index) => <button key={index} onClick={() => scrollTo(index)} className={`w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full transition-all duration-300 ${index === activeIndex ? `bg-[#D4AF37] w-4 sm:w-6` : `${darkMode ? 'bg-gray-600 hover:bg-gray-500' : 'bg-gray-300 hover:bg-gray-400'}`}`} aria-label={`Go to testimonial ${index + 1}`} />)}
          </div>
